...
button.addEventListener("click", (e) => {
    ...
    // タイトルの入力内容が空だったら、メッセージにエラー文を代入
    if (folderTitle.value.length === 0) {
        message.push("フォルダ名は、入力必須です");
    }
    ...
    if (message.length !== 0) {
        ...
        message.forEach(function(value) {
            ...
            sentence.textContent = value;
            // sentence.classList.add("append-content");
            console.log(errorMsg.appendChild(sentence));
            e.preventDefault();
        });
    } else {
        message.push('追加しました')
